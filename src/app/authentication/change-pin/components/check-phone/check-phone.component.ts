...
import { PlatformService, SmsCodeService } from 'src/app/shared/services';
...
export class CheckPhoneComponent implements OnInit, OnDestroy {
  ...
  @Output() companyEmit: EventEmitter<Company> = new EventEmitter<Company>();
  @Output() pinChangeEmit: EventEmitter<PinChange> =
    new EventEmitter<PinChange>();
  ...
  buildFormRegister() {
    this.formRegister = this.formBuild.group({
      phonePrefix: new FormControl(this.phonePrefix, [Validators.required]),
  ...
  checkPhone(pinChange: 'user' | 'company') {
    if (pinChange === 'user') {
      this.userService
        .getUserByPhone(
        ...
        .subscribe({
          next: (response) => {
            if (!response) {
              this.toastr.error(
                this.translate.instant('SING_UP.UNREGISTERED_PHONE')
              );
              return;
            }
            this.user = response;
  ...
  checkPhoneAccount() {
    this.authenticationService
      .checkPhone(this.phonePrefix, this.phone)
      .subscribe({
        next: (response) => {
          if (response.user) {
    ...
    this.smsCodeService.checkCode(code, this.smsCodeId).subscribe({
      next: (response) => {
        if (response) {
          ...
          this.companyEmit.emit(this.company);
          this.pinChangeEmit.emit(this.pinChange);
        }
      },
      error: () => {},
      complete: () => {},
  ...
  checkPinCount() {
    this.pinCount = 0;
    if (this.formCode.controls['code1'].value) this.pinCount += 1;
    if (this.formCode.controls['code2'].value) this.pinCount += 1;
    if (this.formCode.controls['code3'].value) this.pinCount += 1;
    if (this.formCode.controls['code4'].value) this.pinCount += 1;
    if (this.formCode.controls['code5'].value) this.pinCount += 1;
    if (this.formCode.controls['code6'].value) this.pinCount += 1;
  }
