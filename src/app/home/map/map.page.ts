...
export class MapPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(async (params: any) => {
      const businessSuggestion = params.businessSuggestion;
      if (businessSuggestion === 'yes') {
  ...
  goMap() {
    this.navController.navigateForward('map/map-view');
  }
  ...
  async modalQrScan() {
    ...
    //   this.getWalletByUserIdAndProg(dataQrValue.wallet);
    // }
  }
  ...
  //       this.points = (this.amount * response.profile?.pointsPercentage!) / 100;
