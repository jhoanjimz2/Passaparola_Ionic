...
@Component({
  selector: 'app-view-willbuys-popular',
  templateUrl: './view-willbuys-popular.page.html',
  styleUrls: ['./view-willbuys-popular.page.scss'],
  standalone: true,
  imports: [
...
export class ViewWillbuysPopularPage implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const willbuysSub = this.jointlybuyService.allWillbuyPopular().subscribe({
  ...
  searchWillbuys = (params: any): Observable<any> => {
    return this.jointlybuyService.loadWillbuysPopularSearchBar(params);
  }
  ...
  private loadWillbuysPopular(reset = false): void {
    ...
    if (reset) {
      ...
      this.searchKeyword = '';
      this.jointlybuyService.clearWillbuysPopular();
    }
    ...
    const sub = this.jointlybuyService.loadWillbuysPopular(params, accumulate).subscribe({
      next: (respWillbuy) => {
  ...
  loadMoreData(event: any): void {
    ...
    const sub = this.jointlybuyService.loadWillbuysPopular(params, true).subscribe({
      next: (respWillbuy) => {
        const newDataLength = respWillbuy.data?.length || 0;
        if (newDataLength < this.pageSize) {
          this.hasMoreData = false;
