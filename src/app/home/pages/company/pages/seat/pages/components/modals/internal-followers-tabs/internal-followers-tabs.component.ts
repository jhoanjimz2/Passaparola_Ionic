...
@Component({
  selector: 'app-internal-followers-tabs',
  templateUrl: './internal-followers-tabs.component.html',
  styleUrls: ['./internal-followers-tabs.component.scss'],
  imports: [
    IonicModule,
    CommonModule,
...
export class InternalFollowersTabsComponent {
  @Input() id!: string;
  @Input() tab: TabType = 'followers';
  activeTab: TabType = 'followers';

  followersState: LoadState = {
    data: [],
    total: 0,
    limit: 10,
    offset: 1,
  ...
  ngOnInit() {
    ...
    if (!this.restoreStateFromCache('followers')) this.loadData('followers', true);
    if (!this.restoreStateFromCache('following')) this.loadData('following', true);
  }
   ...
   */
  private restoreStateFromCache(type: 'followers' | 'following'): boolean {
    const cacheKey = `followers:${type}:${this.id}:p1`;
    const cached = this.cacheService.get<any>(cacheKey);
    if (!cached) return false;

    const state = type === 'followers' ? this.followersState : this.followingState;
  ...
  loadMore(event: any, type: 'followers' | 'following') {
    this.loadData(type, false, event);
  }

  private loadData(type: 'followers' | 'following', reset = false, event?: any) {
    const state = type === 'followers' ? this.followersState : this.followingState;
    const serviceMethod =
      type === 'followers'
        ? this.followersService.findAllFollowers.bind(this.followersService)
        : this.followersService.findAllFollowing.bind(this.followersService);
